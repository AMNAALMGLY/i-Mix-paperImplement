...
class Npair(pl.LightningModule):
  ...
  def _shared_step(self,batch,batch_idx):
    ...
    if self.use_imix:
      ...
      logits = torch.matmul(F.normalize(self.model(r_mix)), F.normalize(self.model(r_prime)).T) / self.t
      loss= lam * self.criterion(logits, torch.arange(len(x)).to(self.device)) + \
        (1-lam) * self.criterion(logits, randidx)
     
    else:
      logits=torch.matmul(F.normalize(self.model(r)), F.normalize(self.model(r_prime)).T) / self.t
